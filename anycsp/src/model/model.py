...
class ANYCSP(Module):
    ...
    def forward(
        ...
        # num_edges = sum([w['weight'] for _, _, w in graph.edges(data=True)])
        num_edges = graph.number_of_edges()
        edge_indices = torch.tensor([[u,v] for u,v in graph.edges()], device='cuda')
        edge_values = torch.tensor([w['weight'] for u,v,w in graph.edges(data=True)], device='cuda')
        data.init_adj()
        incumbents = [0.]
        timing = [0.]
        ...
        for s in range(steps):
            ...
            if cur_opt < opt:
                ...
                sols = data.best_assignment.clone().detach().view(args.num_boost, -1)  # [B, n]
                different = (sols[:, edge_indices[:, 0]] != sols[:, edge_indices[:, 1]]).int()

                cuts = edge_values * different
                # cuts = edge_values * (sols[:, edge_indices[:, 0]] + sols[:, edge_indices[:, 1]] - 2 * sols[:, edge_indices[:, 0]] * sols[:, edge_indices[:, 1]])
                cuts = cuts.sum(-1)
                cut = cuts.max()
                if verbose:
                    print(cut)
                    # print(liu)
                    # print(num_edges - data.best_num_unsat, num_edges - opt.int().cpu(), sum([w['weight'] for u,v,w in graph.edges(data=True) if data.best_assignment[u]!=data.best_assignment[v]]))
                incumbents.append(-cut.item())
                timing.append(tm.perf_counter() - start_time)
                if keep_time:
